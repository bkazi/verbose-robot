...
#if NUM_RAYS <= 1
      vec4 direction =
          glm::normalize(vec4(vec3(x, y, camera.focalLength) * camera.R, 1));
      color += Light(camera.position, direction, 0);
#else
      for (int i = -NUM_RAYS / 2; i < NUM_RAYS / 2; i++) {
        for (int j = -NUM_RAYS / 2; j < NUM_RAYS / 2; j++) {
          vec4 direction = glm::normalize(
              vec4(vec3((float)x + apertureSize * i,
                        (float)y + apertureSize * j, camera.focalLength) *
                       camera.R,
                   1));
          color += Light(camera.position, direction, 0);
        }
...
void Update(Camera &camera, screen *screen) {
  ...
  camera.R = CalcRotationMatrix(camera.rotation.x, camera.rotation.y,
                                camera.rotation.z);
...
bool ClosestIntersection(vec4 start, vec4 dir,
  ...
  vector<uint32_t> indices = bvh->intersect(start, dir);
  ...
  for (uint i = 0; i < indices.size(); ++i) {
    for (uint j = 0; j < scene[indices[i]]->primitives.size(); ++j) {
      float dist = scene[indices[i]]->primitives[j]->intersects(start, dir);
      if (dist > 0 && dist < closestIntersection.distance) {
        closestIntersection.distance = dist;
...
vec3 Light(const vec4 start, const vec4 dir, int bounce) {
  ...
  if (ClosestIntersection(start + dir * 1e-4f, dir, intersection)) {
    ...
    if (bounce > MIN_BOUNCES &&
        (bounce > MAX_BOUNCES || U > max3(primitive->color))) {
      // terminate
      return vec3(0);
    }

    vec4 hitPos = intersection.position;
    vec4 normal = primitive->getNormal(hitPos);
...
mat3 CalcRotationMatrix(float x, float y, float z) {
  mat3 Rx = mat3(vec3(1, 0, 0), vec3(0, cosf(x), sinf(x)),
                 vec3(0, -sinf(x), cosf(x)));
  mat3 Ry = mat3(vec3(cosf(y), 0, -sinf(y)), vec3(0, 1, 0),
                 vec3(sinf(y), 0, cosf(y)));
  mat3 Rz = mat3(vec3(cosf(z), sinf(z), 0), vec3(-sinf(z), cosf(z), 0),
                 vec3(0, 0, 1));
  return Rx * Ry * Rz;
}
